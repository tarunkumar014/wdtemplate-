...
final class CPRO_Service_MailJet extends CPRO_Service {
	...
	public function render_connect_settings() {
		$mailjet_api_url = add_query_arg( array(), 'https://app.mailjet.com/account/api_keys' );
		ob_start();
		...
		ConvertPlugHelper::render_input_html(
			...
			array(
				...
				/* translators: %s link */
				'description' => sprintf( __( 'Please refer to the following article for more details: <a%1$s rel="noopener">Getting an API key</a>', 'convertpro-addon' ), ' href="' . $mailjet_api_url . '" target="_blank"' ),
			)
		);
		return ob_get_clean();
	}
