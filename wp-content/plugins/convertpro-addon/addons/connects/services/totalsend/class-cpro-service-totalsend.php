...
final class CPRO_Service_TotalSend extends CPRO_Service {
	...
	public function render_connect_settings() {
		$totalsend_api_url = add_query_arg( array(), 'https://app.totalsend.com/app/user/integration/wordpress/' );
		ob_start();
		...
		ConvertPlugHelper::render_input_html(
			...
			array(
				...
				/* translators: %s: Link */
				'description' => sprintf( __( 'Please see <a%1$s rel="noopener">Getting an API details</a> for complete instructions.', 'convertpro-addon' ), ' href="' . $totalsend_api_url . '" target="_blank"' ),
			)
		);
		return ob_get_clean();
	}
	...
	public function subscribe( $settings, $email, $name = false ) {
		...
		if ( false === $result['Success'] ) {
			...
			} elseif ( 9 === $result['ErrorCode'] ) { // phpcs:ignore Generic.CodeAnalysis.EmptyStatement.DetectedElseif
